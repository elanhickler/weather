...
def require_node_graph_mvp_contract() -> None:
    ...
    for snippet in [
        ...
        "<span>Update</span><span>Default</span>",
        "Patch Utility",
        ...
        "nodeSavedPatchesBankStatus",
        "nodeSavedPatchesBankInput",
        "nodeSavedPatchesBankNameInput",
        "nodeSceneOpenSavedPatches",
        "nodeSavedPatchesWindow",
        "nodeSavedPatchesWindowHeading",
        "nodeSavedPatchesPreviousButton",
        "nodeSavedPatchesNextButton",
        "nodeSavedPatchesFitInput",
        "nodeSavedPatchesTagInput",
        "search tag",
        "Search patch explorer by tag",
        "nodeSavedPatchesTagChips",
        "nodeSavedPatchesPatchNameInput",
        "nodeSavedPatchesPatchTagsInput",
        "nodeSavedPatchesInitPatch",
        "nodeSavedPatchesSavePatch",
        "nodeSavedPatchesSetDefault",
        "nodeSavedPatchesLoadPatch",
        "nodeSavedPatchesCopyPatch",
        "nodeSavedPatchesPastePatch",
        "nodeSavedPatchesSaveBank",
        "nodeSavedPatchesLoadBank",
        "nodeSavedPatchesBankFileInput",
        ...
        'aria-label="Post Processing" aria-pressed="false"><span>Post</span><span>Processing</span>',
        ...
        "floating-window-drag-area-1",
        "required-assets-1",
        "wire-feedback-route-1",
        "module-home-actions-1",
        "Module Actions",
        "Metaparameters",
        "Oscilloscope Settings",
        "timing-flat-controls-1",
        ...
        'data-global-scope-input="dotCore1Size" data-global-scope-number-drag="true" value="2"',
        'data-global-scope-input="dotCore1Brightness" data-global-scope-number-drag="true" value="0.23"',
        ...
        "nodeModuleHomeShelf",
        "nodeModuleDepartmentList",
        "nodeModuleDeveloperListShell",
        "nodeModuleDeveloperList",
        "nodeModuleGroups",
        "nodeModuleGroupList",
        "unfinished modules",
        "Search modules...",
        "node-module-store-grid",
        "Module buttons fit horizontally",
        ...
        "missing-sample-dismiss-1",
        "required-assets-1",
        "wire-feedback-route-1",
        "module-home-actions-1",
        "patch-slot-overlap-1",
        "patch-card-even-stroke-1",
        "io-alt-bypass-1",
        "metadata-textarea-resize-1",
        "module-home-actions-1",
    ...
    require(
        ...
        and ">Metaparameter</button>" in scene_context_source,
    ...
    require(
        patch_explorer_source.index('id="nodeSavedPatchesBankInput"') <
        patch_explorer_source.index('id="nodeSavedPatchesSaveBank"') <
        patch_explorer_source.index('id="nodeSavedPatchesLoadBank"')
        and 'class="node-saved-patches-actions" hidden' not in patch_explorer_source
        and 'class="node-saved-patches-command-row bank"' not in patch_explorer_source,
        "patch bank save/load controls should live on the bank number row",
    )
    require(
        "Set Default" in patch_explorer_source
        and 'id="nodeSavedPatchesLoadPatch" type="button" aria-label="Open patch" title="Open patch">Open</button>' in patch_explorer_source
        and patch_explorer_source.index('id="nodeSavedPatchesLoadPatch"') < patch_explorer_source.index('id="nodeSavedPatchesSavePatch"')
        and 'id="nodeSavedPatchesSavePatch" type="button" aria-label="Save patch" title="Save patch">Save</button>' in patch_explorer_source
        ...
        "patch action conglomerate should use compact text action labels",
    )
    require(
        'data-patch-info-field="name"' in patch_explorer_source
        and 'data-patch-info-field="tags"' in patch_explorer_source
        and 'handleNodeGraphSavedPatchInfoInput' in script_sources["./public/node-graph-file-actions.js"]
        and 'document.getElementById("nodeSavedPatchesPatchNameInput").addEventListener("input", handleNodeGraphSavedPatchInfoInput)' in script_sources["./public/node-graph-header-event-bindings.js"],
        "patch explorer name and tags fields should sync into patch metadata",
    )
    require(
        "row.classList.toggle(\"empty\", !patch.filename)" in script_sources["./public/node-graph-file-actions.js"]
        and "name.textContent = patch.name || \"Empty\"" in script_sources["./public/node-graph-file-actions.js"]
        and ".node-demo-patch-row.empty" in style_source,
        "patch explorer should render all 128 slots including empty slots",
    )
    ...
    for snippet in [
        ...
        'createNodeGraphPatchNode("canvas", { alias: "Origin", id: "canvas-origin"',
        ...
        "`/api/patches?bank=${encodeURIComponent(bank)}`",
        ...
        "param.waveform.choices = [Saw, Square, Triangle, Sine, Noise];",
        ...
        "setNodeModuleActionsWindowHeader(moduleMode ? \"MODULE ACTIONS\" : \"WIRE ACTIONS\"",
    ...
    require(
        ...
        and "function nodeGraphModuleScopeShouldPreferOfflineOutputAnalyzer(slot, buffer)" in node_graph_source
        and "const offlineAnalyzer = nodeGraphModuleScopeOfflineOutputAnalyzerBuffer(slot)" in node_graph_source
        and "nodeGraphModuleScopeShouldPreferOfflineOutputAnalyzer(slot, offlineAnalyzer)" in node_graph_source
    ...
    output_scope_source = node_graph_source[
        node_graph_source.find("function nodeGraphModuleScopeOfflineOutputAnalyzerBuffer"):
        node_graph_source.find("function nodeGraphModuleScopeCapturedOutputAnalyzerBuffer")
    ]
    ...
    for snippet in [
        ...
        "--node-scope-port-band-overlap: var(--node-io-section-min-height)",
        ".dsp-node.visual-scope-layout",
        ".visual-scope-layout .node-module-square-scope-window",
        "height: calc(100% + var(--node-scope-port-band-overlap))",
        "align-self: stretch",
        ".node-module-scope-window",
        "height: calc(var(--node-module-scope-height) + var(--node-scope-port-band-overlap))",
        ...
        "width: min(var(--node-module-shop-width, 520px), calc(100vw - 28px))",
        "height: min(var(--node-module-shop-height, 620px), calc(100vh - 24px))",
        ...
        ".node-saved-patches-bank-control",
        ".node-saved-patches-fit-control",
        ".node-saved-patches-tag-filter",
        ".node-saved-patches-tag-chips",
        ".node-saved-patches-tag-chip",
        "--node-saved-patch-columns",
        "container-type: inline-size",
        "grid-auto-rows: calc(100cqw / var(--node-saved-patch-columns, 3))",
        "aspect-ratio: auto",
        "overflow-y: auto",
        "overscroll-behavior: contain",
        ".node-demo-patch-list",
        ".node-demo-patch-row",
        ".node-demo-patch-tags",
        "grid-template-rows: minmax(0, 0.72fr) minmax(0, 2fr) minmax(0, 0.9fr)",
        "align-self: center",
        "align-self: end",
        ...
        ".dsp-node.oscilloscope-hidden .node-module-scope-window",
        ".node-graph-workspace.module-buttons-hidden .dsp-node .node-header-actions",
        ".node-graph-workspace.module-sliders-hidden .node-param-row",
        ...
        ".node-visibility-menu-heading.dragging",
        ...
        "width: min(var(--node-module-actions-width, 180px), calc(100vw - 28px))",
        ...
        "linear-gradient(180deg, rgb(24, 31, 36), rgb(10, 14, 17))",
        ...
        "grid-template-columns: 34px minmax(0, 1fr) 34px",
        ".node-saved-patches-header-tools",
        ".node-saved-patches-drag-handle",
        ".node-saved-patches-step-controls",
        ".node-saved-patches-step-controls button:first-child",
        ".node-saved-patches-step-controls button:last-child",
        ".node-saved-patches-resize-handle",
        "touch-action: none",
        "grid-template-columns: minmax(98px, 1fr) repeat(2, minmax(76px, 0.7fr))",
        ".node-saved-patches-actions .node-saved-patches-bank-control",
        ".node-saved-patches-command-row",
        "grid-template-columns: repeat(6, minmax(0, 1fr))",
        ".node-saved-patches-command-row button + button",
        ...
        "grid-template-columns: minmax(42px, 0.82fr) repeat(7, minmax(36px, 1fr))",
        "flex: 1 1 46%",
